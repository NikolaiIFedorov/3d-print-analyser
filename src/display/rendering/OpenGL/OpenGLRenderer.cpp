...
OpenGLRenderer::OpenGLRenderer(OpenGLRenderer &&other) noexcept
    : window(other.window),
      ...
      pickHighlightCalibInvalidVAO(other.pickHighlightCalibInvalidVAO),
...
void OpenGLRenderer::Shutdown()
{
    ...
    if (pickHighlightCalibInvalidVBO)
        glDeleteBuffers(1, &pickHighlightCalibInvalidVBO);
    if (pickHighlightCalibInvalidVAO)
        glDeleteVertexArrays(1, &pickHighlightCalibInvalidVAO);
    if (pickHighlightCalibInvalidIBO)
...
void OpenGLRenderer::UploadPickHighlightCalibInvalidMesh(const std::vector<Vertex> &vertices,
                                                         const std::vector<uint32_t> &indices)
{
    pickHighlightCalibInvalidIndexCount = static_cast<uint32_t>(indices.size());

    if (pickHighlightCalibInvalidVAO == 0)
        glGenVertexArrays(1, &pickHighlightCalibInvalidVAO);

    glBindVertexArray(pickHighlightCalibInvalidVAO);

    if (pickHighlightCalibInvalidVBO == 0)
        glGenBuffers(1, &pickHighlightCalibInvalidVBO);

    glBindBuffer(GL_ARRAY_BUFFER, pickHighlightCalibInvalidVBO);
    glBufferData(GL_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(vertices.size() * sizeof(Vertex)),
                 vertices.empty() ? nullptr : vertices.data(),
                 GL_DYNAMIC_DRAW);

    if (pickHighlightCalibInvalidIBO == 0)
        glGenBuffers(1, &pickHighlightCalibInvalidIBO);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, pickHighlightCalibInvalidIBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(indices.size() * sizeof(uint32_t)),
                 indices.empty() ? nullptr : indices.data(),
                 GL_DYNAMIC_DRAW);
...
void OpenGLRenderer::DrawPickHighlightCalibInvalid()
{
    if (pickHighlightCalibInvalidIndexCount == 0)
        return;
    ...
    glBindVertexArray(pickHighlightCalibInvalidVAO);
    glDrawElements(GL_TRIANGLES, pickHighlightCalibInvalidIndexCount, GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
