...
**2026-05-11 follow-up:** Pill hover/text used **window-local** coords on `GetWindowDrawList` (screen space) — fixed with `GetCursorScreenPos()` origin. When import completes, **hide the whole Prerequisites `Section`** (`visible = false`) so a zero-height section no longer sits between subtitle and footer (removed double splitter / mystery bars).
