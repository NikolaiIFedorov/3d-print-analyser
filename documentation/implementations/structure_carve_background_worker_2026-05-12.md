...
## Follow-up (2026-05-12, third pass — isolated carve queue)

- **Symptom:** With pathological meshes (e.g. self-intersecting footprint), CGAL can run extremely long or not return inside one solid. The carve job shared `Display::taskRunner` with import and analysis, so **analysis never started** while the worker was stuck, and **`~TaskRunner::join`** on exit waited forever on that worker.
- **Fix:** `BeginStructureStagingSession` now submits to **`StructureCarveTaskRunner()`** — a file-static `TaskRunner` in `display.cpp`, separate from `taskRunner`. It is **intentionally never destroyed** (heap singleton) so app teardown does not join a stuck carve thread; process exit reclaims the OS thread. Import/analysis continue to use `Display::taskRunner`.
- **Caveat:** A **second** carve cannot start on the isolated runner until the **first** worker call returns (single worker, FIFO). Cancelling only affects jobs **between** solids; cancel mid–`TryApplyStructureCarve` still cannot preempt CGAL. Re-entering Structure while one solid hangs may still show no new carve until the hang clears.
- **Docs:** `documentation/Architecture_UIThreadAndWorkers.md` updated.
