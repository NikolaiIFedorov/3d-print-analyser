...
- **Strip→Settings gap:** Removed `gapBelowStripCells` after StatusStrip; shift is `dy = strip.rowSpan` only so outer row bounds abut like other stacked anchors (spacing is only the two panels’ margins, not an extra quarter-cell).
- **Idle status text:** Shows last uploaded main mesh tri+line index counts and tri+line vertex buffer sizes (`OpenGLRenderer` / `SceneRenderer` getters), plus topology `F/E pts sol` (or `topo empty`).
